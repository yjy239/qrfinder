...
void findPossibleQrContours(Mat &binary,vector<vector<Point>> &contoursRes) {
...
void drawMat(vector<vector<Point>> &temp,Mat &src){
    ...
    points.push_back(Point(maxLeft,maxTop));
    points.push_back(Point(maxLeft,maxBottom));
    points.push_back(Point(maxRight,maxBottom));
    points.push_back(Point(maxRight,maxTop));
    contours.push_back(points);

    drawContours(src,contours,0,Scalar(255,0,0),1);
}


extern "C"
JNIEXPORT jobject JNICALL
Java_com_yjy_qrfind_QRFinder_findQR(JNIEnv *env, jclass type, jobject bitmap) {
    ...
    findPossibleQrContours(binary,contoursRes);
    ...
    if(contoursRes.size() > 3){
        ...
        vector<vector<Point>> temp = contoursRes;
...
void push_mat(vector<vector<Point>> &temp,Mat &src,vector<Mat> &resultMats){
    int maxLeft = INT_MAX, maxTop = INT_MAX,maxRight = 0,maxBottom = 0;
    for(int i = 0;i<temp.size();i++){
        vector<Point> t = temp[i];
        for(int j = 0;j<t.size();j++){
    ...
    Mat dst(src,Rect(Point(maxLeft,maxTop),Size(maxRight - maxLeft,maxBottom - maxTop)));
    resultMats.push_back(dst);
}
...
Java_com_yjy_qrfind_QRFinder_getAllQrRect(JNIEnv *env, jclass type, jobject metrics,
                                            jobject bitmap, jboolean isBinary) {
    ...
    if(contoursRes.size() > 3){
        ...
        for(auto it = resultSet.begin();it != resultSet.end();it++){
            ...
            for(auto it = resultSet.begin();it != resultSet.end();it++){
                ...
                if(isBinary){
                    push_mat(temp,binary,resultMats);
                }else{
                    push_mat(temp,src,resultMats);
                }
        ...
        if(isBinary){
            push_mat(contoursRes,binary,resultMats);
        }else{
            push_mat(contoursRes,src,resultMats);
        }
    ...
    _array = env->NewObjectArray(resultMats.size(),bitmap_clazz,NULL);
    ...
    jmethodID create_bitmap_mid = env->GetStaticMethodID(bitmap_clazz,
                                                         "createBitmap","(Landroid/util/DisplayMetrics;IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;");


    jclass bitmap_config_clazz = env->FindClass("android/graphics/Bitmap$Config");

    jfieldID  config_8888_id = env->GetStaticFieldID(bitmap_config_clazz,"ARGB_8888","Landroid/graphics/Bitmap$Config;");
    jobject config_8888 = env->GetStaticObjectField(bitmap_config_clazz,config_8888_id);


    for(int i = 0;i<resultMats.size();i++){
        jobject bm = env->CallStaticObjectMethod(bitmap_clazz,create_bitmap_mid,metrics,
                                                 resultMats[i].cols,resultMats[i].rows,config_8888);
        cv_helper::matTobitmap(env,bm,resultMats[i],false);
        env->SetObjectArrayElement(_array ,i,bm);
    }
